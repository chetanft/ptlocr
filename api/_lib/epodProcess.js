...
function classifyItem(extracted, confidence, matchedShipment, selectedAwbs, _fileName) {
  ...
  if (extracted.remarks || extracted.condition_notes) {
    ...
    if (damageKeywords.some((keyword) => text.toLowerCase().includes(keyword))) {
...
function countMatchedAwbs(items) {
  return new Set(
    items
      ...
      .map((item) => normalizeAwb(item.awbNumber)),
  ).size;
}
...
export async function processEpodBatch(files, selectedAwbs, apiKey, provider = 'openai') {
  const items = [];
